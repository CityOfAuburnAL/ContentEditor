class ContentTools.CustomLinkDialog extends ContentTools.DialogUI
	...
	constructor: (href='', target='') ->
		...
		# Now we determin what kind of link it is if the link is populated
		@_mailTo = (@_fullURL.indexOf('mailto:') == 0 ? 'mailto:' : '')
		@_pdf = (@_fullURL.indexOf('.pdf') != -1 ? '.pdf' : '')
	...
	mount: () ->
		...
		domTools.appendChild(@_domMailTo)
		...
		@_domInput.setAttribute('class', 'ct-pdf-dialog__file-upload')
	...
	_addDOMEventListeners: () ->
		...
		@_domCancelUpload.addEventListener 'click', (ev) =>
			@dispatchEvent(@createEvent('pdfuploader.cancelupload'))

		# Clear - probably will be removed
		@_domClear.addEventListener 'click', (ev) =>
			@clear()
			@dispatchEvent(@createEvent('pdfuploader.clear'))

		# MailTo link
		@_domMailTo.addEventListener 'click', (ev) =>
			@state('mailTo')
			@dispatchEvent(@createEvent('contactmanager.init'))
		# PDF link
		@_domPDF.addEventListener 'click', (ev) =>
			@state('pdf')
			@dispatchEvent(@createEvent('pdfuploader.upload'))
